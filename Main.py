...
📁 */projects* - Показать все мои проекты
...
💡 *Совет:* Также ты можешь просто ввести название проекта и узнать о нем информацию!
⚡ *Бот автоматически скрывает клавиатуру после выбора
""")
...
def add_description_handler(message):
    ...
    if projects:
        ...
        bot.register_next_step_handler(message, add_description_step_2, projects=projects)
    else:
        no_projects(message)

def add_description_step_2(message, projects):
    ...
    if project_name not in projects:
        bot.send_message(message.chat.id, 'У тебя нет такого проекта, попробуй еще раз!) Выбери проект для добавления описания', reply_markup=gen_markup(projects))
        bot.register_next_step_handler(message, add_description_step_2, projects=projects)
        return
    
    bot.send_message(message.chat.id, 'Введите описание проекта:')
    bot.register_next_step_handler(message, add_description_step_3, project_name=project_name)

def add_description_step_3(message, project_name):
